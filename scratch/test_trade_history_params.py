...
def test_history(params):
    ...
    print(f"Params: {list(params.keys())} | Values: {params} | Code: {response.status_code} | Resp: {response.text[:200]}")

print("--- Testing trades/history ---")
test_history({"pair": "XRPMYR"})
test_history({"pair_name": "XRPMYR"})
test_history({"symbol": "XRPMYR"})
test_history({"pair": "XRPMYR", "page": 1, "limit": 10})
test_history({"pair_name": "XRPMYR", "page": 1, "limit": 10})
