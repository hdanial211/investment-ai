...
def test_users_orders(params):
    ...
    if response.status_code == 200:
        print("Resp (first 200 chars):", response.text[:200])
    else:
        print("Resp:", response.text)

print("--- Testing combinations ---")
test_users_orders({"pair": "XRPMYR", "page": 1, "limit": 10})
test_users_orders({"pair": "XRPMYR", "page": "1", "limit": "10"})
test_users_orders({"pair": "XRPMYR", "page": 1, "rows": 10})
test_users_orders({"pair": "XRPMYR", "page": "1", "rows": "10"})
test_users_orders({"pair": "XRPMYR", "page": 1, "limit": 10, "status": "active"})
test_users_orders({"pair": "XRPMYR", "page": "1", "limit": "10", "status": "active"})
test_users_orders({"pair": "XRPMYR", "page": 1, "rows": 10, "status": "active"})
test_users_orders({"pair": "XRPMYR", "page": "1", "rows": "10", "status": "active"})
